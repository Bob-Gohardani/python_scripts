...
"""
A sub-class must be substitutable for its super-class. nThe aim of this
principle is to ascertain that a sub-class can assume the place of its
super-class without errors.  

If the code finds itself checking the type of class, it must have violated this principle.
...
super-class type (Animal type) or sub-class type(Pigeon).
"""

def animal_leg_count_fixed(animals: list):
    for animal in animals:
        print(animal.leg_count())  # here if the animal type is Lion, it will call leg_count for Lion instead of Animal

animal_leg_count_fixed(animals)
